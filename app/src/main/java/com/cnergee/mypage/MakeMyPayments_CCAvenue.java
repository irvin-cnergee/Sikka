...
public class MakeMyPayments_CCAvenue extends BaseActivity implements OnCancelListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		linnhome.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				MakeMyPayments_CCAvenue.this.finish();
				Intent i = new Intent(MakeMyPayments_CCAvenue.this, IONHome.class);
				startActivity(i);
		...
		linnprofile.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				MakeMyPayments_CCAvenue.this.finish();
				Intent i = new Intent(MakeMyPayments_CCAvenue.this, Profile.class);
				startActivity(i);
				overridePendingTransition(R.anim.slide_in_left,
						R.anim.slide_out_right);
				BaseApplication.getEventBus().post(
		...
		linnnotification.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				MakeMyPayments_CCAvenue.this.finish();
				Intent i = new Intent(MakeMyPayments_CCAvenue.this,
		...
		linnhelp.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				MakeMyPayments_CCAvenue.this.finish();
				Intent i = new Intent(MakeMyPayments_CCAvenue.this, HelpActivity.class);
				startActivity(i);
				overridePendingTransition(R.anim.slide_in_left,
						R.anim.slide_out_right);
				BaseApplication.getEventBus().post(
	...
	private class GetSignatureFromMerchantServer extends
		...
		protected void onPreExecute() {
			super.onPreExecute();
			if(is_activity_running){
				mProgressHUD = ProgressHUD.show(MakeMyPayments_CCAvenue.this, "Loading...",
					true, true, this);
		...
		protected void onPostExecute(String result) {
			...
			Intent launchIntent = new Intent(MakeMyPayments_CCAvenue.this,
					WebViewScreen.class);
	...
	private void changeTheContentView() {
		if (response != null && response.size() > 0) {
			TextView heading = new TextView(MakeMyPayments_CCAvenue.this);
	...
	private void addFieldsToTheMainLayout(String fieldName, String viewHeading) {
		...
		TextView textView = new TextView(MakeMyPayments_CCAvenue.this);
				...
				try {
					...
					if(TrackId.length()>0){
						...
						/*if(Utils.pg_sms_request){
							insert_uid_trackid();
							AlertsBoxFactory.showAlert("From Notification List", MakeMyPayments_CCAvenue.this);
						}
						else{
							AlertsBoxFactory.showAlert("From Normal", MakeMyPayments_CCAvenue.this);
						}*/
			...
			try {
				PaymentGatewayCaller adjCaller = new PaymentGatewayCaller(
						...
						getApplicationContext().getResources().getString(
								R.string.SOAP_URL), getApplicationContext()
								.getResources().getString(
										R.string.METHOD_GET_TRANSACTIONID_WITH_BANK_NAME),"CA");
	...
	private class InsertBeforePayemnt extends AsyncTask<String, Void, Void>
		...
		protected void onPostExecute(Void unused) {
			...
			if (rslt.trim().equalsIgnoreCase("ok")) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
					new Get_CCAvenue_Signature().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, (String)null);
				}
				else{
				new Get_CCAvenue_Signature().execute();
				}
		...
		protected Void doInBackground(String... params) {
			try {
				...
				paymentsObj.setPackageName(txtnewpackagename.getText()
						.toString());
	...
	private class RenewalProcessWebService extends
		...
		protected Void doInBackground(String... params) {
			try {
				...
				paymentsObj.setPaidAmount(Double.parseDouble(txtnewamount
	...
	public void showPaymentDetailsDialog(AdditionalAmount additionalAmount) {
		if (additionalAmount != null) {
			final Dialog dialog = new Dialog(MakeMyPayments_CCAvenue.this);
			...
			WindowManager w = ((Activity) MakeMyPayments_CCAvenue.this)
					.getWindowManager();
	...
	public void seamless_integration(){
		is_member_details=true;
		if(Double.valueOf(txtnewamount.getText().toString())>0){
		if(memberDetails!=null&&memberDetails.getMemberName().length()>0){
			
			if(TrackId!=null&&TrackId.length()>0){
				MakeMyPayments_CCAvenue.this.finish();
				Intent intent_show_payopt= new Intent(MakeMyPayments_CCAvenue.this, PaymentOptionSelection_Activity.class);								
				
				
				intent_show_payopt.putExtra("MemberDetails", memberDetails);
				intent_show_payopt.putExtra("TrackId", TrackId);
				intent_show_payopt.putExtra("Amount", txtnewamount.getText().toString());
				intent_show_payopt.putExtra("PackageName", txtnewpackagename.getText().toString());
				startActivity(intent_show_payopt);
			}
			else{
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
					getpaymentgatewaysdetails = new PaymentGateWayDetails();
		...
		if(is_activity_running)
			AlertsBoxFactory.showAlert("Invalid Amount", MakeMyPayments_CCAvenue.this);
}
	}
	
	public class Get_CCAvenue_Signature extends AsyncTask<String, Void, Void> implements OnCancelListener{
	...
	String get_SignResult="",get_Sign_Response="";
		...
		protected void onPreExecute() {
			...
			super.onPreExecute();
			if(is_activity_running)
			mProgressHUD=ProgressHUD.show(MakeMyPayments_CCAvenue.this, getString(R.string.app_please_wait_label), true, false, this);
		}
		
		@Override
		protected Void doInBackground(String... params) {
			// TODO Auto-generated method stub
			Get_CCAvenue_SignatureSOAP get_CCAvenue_SignatureSOAP= new Get_CCAvenue_SignatureSOAP(getString(R.string.WSDL_TARGET_NAMESPACE), getString(R.string.SOAP_URL), getString(R.string.METHOD_GET_CCAVENUE_SIGNATURE));
			try {
				get_SignResult=get_CCAvenue_SignatureSOAP.getSignature(String.valueOf(memberid), txtnewamount.getText().toString()	, TrackId);
				get_Sign_Response=get_CCAvenue_SignatureSOAP.getResponse();
			} catch (SocketTimeoutException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (SocketException e) {
		...
		protected void onPostExecute(Void result) {
			...
			if(get_SignResult!=null&&get_SignResult.length()>0&&get_SignResult.equalsIgnoreCase("ok")){
				if(get_Sign_Response!=null&&get_Sign_Response.length()>0){
					Utils.log("CCAvenueSignature Response","is"+get_Sign_Response);					
					parse_CCAvenueSignature_Response(get_Sign_Response);
				}
				else{
					if(is_activity_running)
					iError.display();
	...
	public void parse_CCAvenueSignature_Response(String response){
		try {
			...
			if(mainJsonObject.has("NewDataSet")){
				...
				if(newdatasetJson.has("Table1")){
					...
					JSONObject tableJson=newdatasetJson.getJSONObject("Table1");
					if(tableJson.has("TrackId"))
						TrackId=tableJson.getString("TrackId");
					else
						showExitAlert("Please try Again \n  We are unable to process your request", MakeMyPayments_CCAvenue.this);
					cca_access_code=tableJson.getString("AccessCode");
					cca_enc_req=tableJson.getString("EncRequest");
					cca_merchant_id=tableJson.getString("MerchantId");
					cca_working_key=tableJson.getString("WorkingKey");
					cca_request=tableJson.getString("ccaRequest");
					MakeMyPayments_CCAvenue.this.finish();
					
					Intent call_webview= new Intent(MakeMyPayments_CCAvenue.this, WebView_CCAvenueActivity.class);
					call_webview.putExtra(AvenuesParams.ACCESS_CODE, cca_access_code);
					call_webview.putExtra(AvenuesParams.ENC_VAL, cca_enc_req);
					call_webview.putExtra(AvenuesParams.MERCHANT_ID, cca_merchant_id);
					call_webview.putExtra(AvenuesParams.WORKING_KEY, cca_working_key);
					call_webview.putExtra(AvenuesParams.AMOUNT, txtnewamount.getText().toString());
					call_webview.putExtra("TrackId", TrackId);					
					call_webview.putExtra(AvenuesParams.CCA_REQUEST, cca_request);
					call_webview.putExtra("additional_amount", additionalAmount);
					call_webview.putExtra("Changepack", Changepack);
	 ...
	 public static  void showExitAlert(String message, Context ctx){
	 ...
	 public void insert_uid_trackid(){
	...
	getApplicationContext().getResources().getString(
							...
							.getResources().getString(
									R.string.METHOD_INSERT_SMS_UNIQUEID_TRACKID));
			
			SoapObject request = new SoapObject(getString(R.string.WSDL_TARGET_NAMESPACE), getString(R.string.METHOD_INSERT_SMS_UNIQUEID_TRACKID));
			...
			pi = new PropertyInfo();
			pi.setName("SMSUniqueid");
			pi.setValue(Utils.pg_sms_uniqueid);
			pi.setType(String.class);
			request.addProperty(pi);
			...
			final ProgressHUD	prg_bar = ProgressHUD.show(MakeMyPayments_CCAvenue.this,getString(R.string.app_please_wait_label), true,true,this);
			new CommonAsyncTask(MakeMyPayments_CCAvenue.this){
				...
				protected void onPostExecute(ArrayList<String> result) {
					...
					if(result.get(0).equalsIgnoreCase("OK")){
						MakeMyPayments_CCAvenue.this.finish();
						Intent details_intent=new Intent(MakeMyPayments_CCAvenue.this, MakeMyPayments_CCAvenue.class);
						...
						AlertsBoxFactory.showAlert("Web Service Not Executed", MakeMyPayments_CCAvenue.this);
					}
				}
			}.execute(commonSOAP);
