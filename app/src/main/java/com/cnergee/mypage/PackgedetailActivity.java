...
//    List<Integer> unsortList = new ArrayList<Integer>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        exlv_PackageList = findViewById(R.id.exlv_PackageList);
        tv_marquee.setSelected(true);


        sharedPreferences_renewal = getString(R.string.shared_preferences_renewal);
        SharedPreferences sharedPreferences1 = getApplicationContext().getSharedPreferences(sharedPreferences_renewal, 0);
...
//            unsortList.sort(Comparator.<Integer>reverseOrder());
            ...
            exlv_PackageList.setAdapter(  new CustomGridAdapter( this,unsortList, hm_packge_list));
            exlv_PackageList.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
                @Override
                public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {


                    Toast.makeText(PackgedetailActivity.this, "Test", Toast.LENGTH_LONG).show();
...
//            }
        ...
        for (int i = 0; i < final_pkg_list.size(); i++) {
            list.add(final_pkg_list.get(i).getSrno());
...
//           list.add(Integer.parseInt(String.valueOf(final_pkg_list.get(i).getSrno())));
        ...
        for (int k = 0; k < list.size(); k++) {
            for (int i = 0; i < final_pkg_list.size(); i++) {
                if (list.get(k).equals(final_pkg_list.get(i).getSrno())) {
...
//        gridview.setAdapter(  new CustomGridAdapter( this, last_final_pkg));
        Log.e("unsortList:=", ":" +unsortList+"----"+ hm_packge_list.get(unsortList.get(0)));
