...
public class PAckageDetailSOAP {
	...
	public void setPackageDetails(SoapObject response,
		...
		packagedetails.setPackageName(response.getPropertyAsString("CurrentPlan")
				.toString());
        packagedetails.setSid(Integer.parseInt(response.getPropertyAsString("SId")
                .toString()));
		...
		if(response.hasProperty("NotificationMsg"))
			packagedetails.setNotificationMsg(response.getPropertyAsString("NotificationMsg").toString());
		else
			packagedetails.setNotificationMsg("");
		...
		if(response.hasProperty("ICICI")){
			//packagedetails.setIs_icici(Integer.parseInt(response.getPropertyAsString("ICICI").toString()));
			packagedetails.setIs_icici(0);

		}else{
			packagedetails.setIs_icici(0);
		}
		//if(response.hasProperty("SubPaisa")){
		//packagedetails.setIs_icici(Integer.parseInt(response.getPropertyAsString("ICICI").toString()));
		packagedetails.setIs_subpaisa(1);

	/*}else{
		packagedetails.setIs_subpaisa(0);
	}*/
