...
public class MemberDetailCaller extends Thread{
	...
	public void run() {
		...
		try {
            ...
            if(Utils.is_atom){
                MakeMyPayment_AtomFragment.rslt = MemberSoap.CallSearchMemberSOAP(memberid);
                MakeMyPayment_AtomFragment.mapMemberDetails = MemberSoap.getMapMemberDetails();
            }

			
			if(isAllData()){
				ProfileFragment.rslt = MemberSoap.CallSearchMemberSOAP(
						memberid);
                ProfileFragment.mapMemberDetails = MemberSoap
						.getMapMemberDetails();
			...
			Profile.rslt = "Internet connection not available!!";
                ...
                if(!Utils.is_atom){
//                    MakeMyPayment_AtomFragment.rslt = "Internet connection not available!!";
                }
				if(isTopup_flag())
			...
			Profile.rslt = "Internet connection not available!!";
				...
				MakeMyPaymentsTopUp.rslt = "Internet connection not available!!";
                    ...
                    if(!Utils.is_atom){
//                        MakeMyPayment_AtomFragment.rslt = "Invalid web-service response.<br>"+e.toString();
                    }
						else{
							MakeMyPayments_CCAvenue.rslt = "Internet connection not available!!";
						}
