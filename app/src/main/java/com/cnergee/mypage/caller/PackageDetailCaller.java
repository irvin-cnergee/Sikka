	...
	public void run() {
		...
		try {
			...
			if(IONHome.is_home_running){
                ...
                HomeFragment.rslt = PackageSoap.CallSearchMemberSOAP(
					memberloginid);
                HomeFragment.mapPackageDetails = PackageSoap
		...
		/*	MyappDataUsage.rslt = PackageSoap.CallSearchMemberSOAP(
			...
			MyappDataUsage.mapPackageDetails = PackageSoap
					.getMapPackageDetails();*/
			if(RenewPackageFragment.is_renew_running){
                RenewPackageFragment.rslt = PackageSoap.CallSearchMemberSOAP(memberloginid);
                RenewPackageFragment.mapPackageDetails = PackageSoap
					.getMapPackageDetails();
			}
			...
			e.printStackTrace();
			if(IONHome.is_home_running)
                HomeFragment.rslt = "Internet connection not available!!";
			...
			e.printStackTrace();
			if(IONHome.is_home_running)
                HomeFragment.rslt = "Invalid web-service response.<br>"+e.toString();
			//MyappDataUsage.rslt = "Internet connection not available!!";
			if(RenewPackage.is_renew_running)
