...
import com.cnergee.fragments.PaymentHistoryFragment;
...
public void run() {
		...
		try{
			...
			PaymentHistoryFragment.rslt = paymentSoap.setComplaintList(getMemberid());
            PaymentHistoryFragment.paymentlist = paymentSoap.getPaymentList();
			
		}catch (SocketException e) {
			e.printStackTrace();
            PaymentHistoryFragment.rslt = "Internet connection not available!!";
		}catch (SocketTimeoutException e) {
			e.printStackTrace();
            PaymentHistoryFragment.rslt = "error";
		}catch (Exception e) {
            PaymentHistoryFragment.rslt = "Invalid web-service response.<br>"+e.toString();
		}
